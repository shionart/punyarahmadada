...
def indeks (request):
	...
	if request.method == 'POST':
		...
		# fix me, menggunakan parameter input
...
def detail (request):
	with open('data_blending.json') as f:
...
def koef (request):
	konteks = {}
	return render(request, 'input_koefisien.html', konteks)
...
def insertDb(request):
	jsonnya = {
		...
		'koefPembebanan': {'a' : float(request.POST['a']), 'b' : float(request.POST['b']),'c' : float(request.POST['c']) }
	}
	print(jsonnya)
	try:
		db = dbconnect()
		cursor = db.cursor()
		cursor.execute("""
	    INSERT INTO nemesys(json_data) \
	    VALUES (%s) """, (jsonnya))
		cursor.close()
		return render(request, 'input_koefisien.html',
